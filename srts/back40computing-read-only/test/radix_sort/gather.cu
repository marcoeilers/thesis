...
std::pair<timeval, timeval> sortGather(int *h_keys, int *h_values, int *h_result, int num_elements, bool keys_only)
{
	typedef int KeyType;
...
//printf("error after malloc %i\n", cudaGetLastError());
        gather(d_keys, double_buffer.d_values[double_buffer.selector], d_result, num_elements);
...
int main()
{
	...
	result = sortGather<START, BITS>(h_keys, h_values, h_result, num_elements, false);
