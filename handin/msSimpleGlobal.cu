...
//        int threadspb = blockDim.x;
		...
		int currentBucketVal = myGlobalBuckets[currentLabel * pitchByInt];
                myResult[offset + i] = currentBucketVal;
                myGlobalBuckets[currentLabel * pitchByInt] = currentBucketVal + myValues[i];
        }
...
{
        myResult[i] = bucketSums[blockIdx.x + (myIndices[i]) * pitchByInt] + myResult[i]; //bins[myIndices[i]];
}

}

__global__
void addRemainderSharedShared(int *bucketSums, int *indices, int *result, int segLength, size_t pitch)
{
int pitchByInt = pitch / sizeof(int);
...
{
	buckets[i] = bucketSums[blockIdx.x + i * pitchByInt];
}
__syncthreads();
...
for (int i = threadIdx.x; i < segLength; i+= nThreads)
{
        myResult[i] =  buckets[myIndices[i]] + myResult[i]; //bins[myIndices[i]];
}
...
int shmemPerBlock = (2 * NT * VT1 * sizeof(int));
int blocksPerSm = min(16, (int) (props.sharedMemPerBlock / shmemPerBlock));
...
cudaMemset(allBuckets, pitch * BINS, 0);
