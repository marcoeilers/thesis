...
currentSegment = blockIndices[0] / 256;
currentTodo = currentSegment == (1 << OFFSETS_EXP) - 1 ? todopb :  min(todopb, blockBuckets[currentSegment + 1] - offset);
/*
...
int num_elements = ((1 << EXP) / totalBlocks) * totalBlocks; 
...
printf("Random %i buckets, ns/int %f\n", (1 << BIN_EXP), ((after.tv_sec - before.tv_sec) * 1e9 + (after.tv_usec - before.tv_usec) * 1e3) / (num_elements * NO_RUNS));
