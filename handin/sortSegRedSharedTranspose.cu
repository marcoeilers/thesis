...
__global__ void blockSegRed(int *values, int *labels, int *allBuckets, int hists)
{
	...
	typedef cub::BlockLoad<int*, THREADS_PER_BLOCK, ITEMS_PER_THREAD, cub::BLOCK_LOAD_TRANSPOSE> BlockLoad;
	typedef mgpu::CTASegScan<THREADS_PER_BLOCK, mgpu::plus<int> > SegScan;
//	typedef mgpu::CTASegReduce<THREADS_PER_BLOCK, ITEMS_PER_THREAD, false, int, mgpu::plus<int> > SegReduce;
	typedef cub::BlockRadixSort<int, THREADS_PER_BLOCK, ITEMS_PER_THREAD, int> BlockRadixSortT;
...
//if (threadIdx.x == 0)        printf("for %i threads and %i bit radix sort, needs %i of temp storage; segscan need %i\n", THREADS_PER_BLOCK, 4, sizeof(BlockRadixSortT::TempStorage), sizeof(SegScan::Storage));
	...
	int myValues[ITEMS_PER_THREAD];

//	int *sharedLabels = &shared.labelValues[0];
//	int *sharedValues = &shared.labelValues[THREADS_PER_BLOCK * ITEMS_PER_THREAD];
/*
	for (int i = 0; i < ITEMS_PER_THREAD; i++)
	{
		int index = i * THREADS_PER_BLOCK + tid;
		myLabels[i] = ourLabels[index];
		myValues[i] = ourValues[index];
	}
*/
	BlockLoad(shared.load).Load(ourLabels, myLabels);
	__syncthreads();
	BlockLoad(shared.load).Load(ourValues, myValues);
	__syncthreads();
	BlockRadixSortT(shared.sort).Sort(myLabels, myValues, 0, BIN_BITS);

	__syncthreads();

	shared.firstVals[tid] = tid == 0 ? BUCKETS : myLabels[0];
	
	__syncthreads();
//	myLabels[ITEMS_PER_THREAD] = shared.firstVals[(tid + 1) % THREADS_PER_BLOCK];
	...
	#pragma unroll
	for(int i = 0; i < ITEMS_PER_THREAD; ++i) {
		x = i ? x + myValues[i] : myValues[i];
		localScan[i] = x;
		int first = next;
		next = i < (ITEMS_PER_THREAD - 1) ? myLabels[i+1] : shared.firstVals[(tid + 1) % THREADS_PER_BLOCK];
		bool thisF = first != next;
		if(thisF) x = 0;
		f = f || thisF;
		myValues[i] = thisF;
	}
//if (blockIdx.x < 20) printf("thread %i has x %i and f %i\n", tid, x, f);
	__syncthreads();
	int carryOut;
	...
	for (int i = 0; i < ITEMS_PER_THREAD; i++)
	{
		...
		if (myValues[i])
		{
//			if (x2 != 0 && blockIdx.x < 20) printf("thread %i writing partial result %i for bucket %i, carryIn is %i, local scan is %i, x was %i, f was %i\n", tid, x2, myLabels[i], carryIn, localScan[i], x, f);
			myResult[myLabels[i] * hists] = x2;
			carryIn = 0;
		}
	}
/*
	for (int i = 0; i < ITEMS_PER_THREAD; i++)
	{
		int index = tid * ITEMS_PER_THREAD + i;
		sharedLabels[index] = myLabels[i];
		sharedValue[index] = myValues[i];
	}
	__syncthreads();
...
int main()
{
	...
	unsigned int num_elements = ((1 << EXP) / (THREADS_PER_BLOCK * ITEMS_PER_THREAD * 8)) * (THREADS_PER_BLOCK * ITEMS_PER_THREAD * 8);
...
//	printf("last error is %i\n", cudaGetLastError());
	...
	printf("Rand %i\t%i\t%i\t%f\t%f\t%f\t%i threadspb\n", BUCKETS, num_elements, BITS, time1 / (num_elements * NO_RUNS), time2 / (num_elements * NO_RUNS), (time1 + time2) / (num_elements * NO_RUNS), THREADS_PER_BLOCK);
}
...
//      printf("last error is %i\n", cudaGetLastError());
        ...
        float time1 = result.first.tv_sec * 1e9 + result.first.tv_usec * 1e3;
        float time2 = result.second.tv_sec * 1e9 + result.second.tv_usec * 1e3;
        printf("Deg %i\t%i\t%i\t%f\t%f\t%f\t%i threadspb\n", BUCKETS, num_elements, BITS, time1 / (num_elements * NO_RUNS), time2 / (num_elements * NO_RUNS), (time1 + time2) / (num_elements * NO_RUNS), THREADS_PER_BLOCK);
