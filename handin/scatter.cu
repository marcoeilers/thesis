...
//printf("error after malloc %i\n", cudaGetLastError());
        scatter(double_buffer.d_keys[double_buffer.selector], double_buffer.d_values[double_buffer.selector], d_result, num_elements);
...
*/
	...
	printf("%i\t%i\t%f\t%f\t%f\n", BUCKETS, BITS, time1 / num_elements, time2 / num_elements, (time1 + time2) / num_elements);
