...
struct Enactor
{
	...
	struct IteratePasses
	{
			...
			Enactor &enactor)
		{
			...
			do {
printf("dispatch pass current_bit bits_remaining\t%i\t%i\n", CURRENT_BIT, BITS_REMAINING);
	...
	/**
	 * Marco's single sort run call
	 */
/*	template <int START_BIT, int NO_BITS, typename DoubleBuffer>
	cudaError_t OneRunSort(
		DoubleBuffer& 	problem_storage,
		int		num_elements)
	{
		typedef ProblemInstance<DoubleBuffer, int> ProblemInstance;
                ...
                if (cuda_props.kernel_ptx_version >= 200)
                {
                        return IteratePasses<200, ProblemInstance, LARGE_PROBLEM, NO_BITS, START_BIT>::DispatchPass(problem_instance, *this);
                }
                else if (cuda_props.kernel_ptx_version >= 130)
                {
                        return IteratePasses<130, ProblemInstance, LARGE_PROBLEM, NO_BITS, START_BIT>::DispatchPass(problem_instance, *this);
                }
                else
                {
                        return IteratePasses<100, ProblemInstance, LARGE_PROBLEM, NO_BITS, START_BIT>::DispatchPass(problem_instance, *this);
                }
