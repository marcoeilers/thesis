...
return (1 + (int)
        (N *
                pow( (rand() / (float) RAND_MAX), log(h)/log(1.0-h) ) ));
}
...
for (i = threadIdx.x; i + (3 * THREADS_PER_BLOCK) < todopb; i += 4 * THREADS_PER_BLOCK)
{
	...
	int curVal = 1 + (curInd == curInd2);
	int curVal2 = 1 - (curInd == curInd2);
	curInd2 = (curInd2 + (threadIdx.x * (curInd == curInd2))) % BUCKETS;
	...
	atomicAdd(&myBuckets[curInd2 * HISTS_PER_BLOCK], curVal2);
        ...
        int curVal3 = 1 + (curInd3 == curInd4);
        int curVal4 = 1 - (curInd3 == curInd4);
        curInd4 = (curInd4 + (threadIdx.x * (curInd3 == curInd4))) % BUCKETS;
...
{
	int curInd = blockIndices[i];
	atomicAdd(&myBuckets[curInd * HISTS_PER_BLOCK], 1);
	i += THREADS_PER_BLOCK;
}
...
int available_indices[BUCKETS];
for (int i = 0; i < BUCKETS; i++)
	available_indices[i] = rand() % BUCKETS;



for (int i = 0; i < num_elements; i++)
{
	indices[i] = available_indices[selfsimilar(BUCKETS, SKIM)];	
//	indices[i] = 0;
